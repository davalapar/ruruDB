...
class Database {
    ...
    internalSaveLoop() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const [mainFd, tempFd, oldFd] = yield Promise.all([
                open(this.main, 'r+'),
                open(this.temp, 'r+'),
                open(this.old, 'r+')
            ]);
            while (this.queue.length > 0) {
                ...
                try {
                    ...
                    yield ftruncate(tempFd);
                    yield write(tempFd, dataString, 0, 'utf8');
                    const oldStat = yield fstat(oldFd);
                    if (oldStat.size > 0) {
                        ...
                        if (current.valueOf() - modified.valueOf() >= this.snapshotInterval
                            ...
                            yield read(mainFd, oldContent, 0, oldStat.size, null);
                            yield writeFile(snapshot, oldContent, 'utf8');
                            this.lastSnapshotTimestamp = current.valueOf();
                        }
                    }
                    const mainStat = yield fstat(mainFd);
                    const mainContent = Buffer.alloc(mainStat.size);
                    yield read(mainFd, mainContent, 0, mainStat.size, null);
                    yield ftruncate(oldFd);
                    yield write(oldFd, mainContent, 0, mainStat.size, 0);
                    yield ftruncate(mainFd);
                    yield write(mainFd, dataString, 0, 'utf8');
