...
class Query {
    ...
    gt(field, value) {
        this.resultItems = this.resultItems.filter(item => Number.isFinite(item[field]) && item[field] > value);
        return this;
    }
    gte(field, value) {
        this.resultItems = this.resultItems.filter(item => Number.isFinite(item[field]) && item[field] >= value);
    ...
    has(field, value) {
        this.resultItems = this.resultItems.filter(item => Array.isArray(item[field]) && item[field].includes(value));
        return this;
    }
    hasAnyOf(field, values) {
        this.resultItems = this.resultItems.filter(item => Array.isArray(item[field]) && values.some(value => item[field].includes(value)));
        return this;
    }
    hasAllOf(field, values) {
        this.resultItems = this.resultItems.filter(item => Array.isArray(item[field]) && values.every(value => item[field].includes(value)));
        return this;
    }
    hasNoneOfAny(field, values) {
        this.resultItems = this.resultItems.filter(item => Array.isArray(item[field]) && values.some(value => item[field].includes(value) === false));
        return this;
    }
    hasNoneOfAll(field, values) {
        this.resultItems = this.resultItems.filter(item => Array.isArray(item[field]) && values.every(value => item[field].includes(value) === false));
        return this;
    }
    select(fields) {
    ...
    results() {
        ...
        const ids = new Array(this.resultItems.length);
        const items = new Array(this.resultItems.length);
        for (let i = 0, l = this.resultItems.length; i < l; i += 1) {
            const item = cloneDeep_1.default(this.resultItems[i]);
            ids[i] = this.resultItems[i].id;
...
class TransactionTable {
    ...
    fetchItem(id) {
        ...
        if (this.index.has(id)) {
            ...
            const item = cloneDeep_1.default(realItem);
            item[Tracker] = id;
            this.index.set(id, item);
            const copy = cloneDeep_1.default(item);
            copy[Tracker] = id;
    ...
    removeItem(item) {
        const id = item[Tracker];
        if (id === undefined)
            throw Error('Invild "Item", "id" Symbol is undefined.');
        if (this.index.has(id)) {
...
class PseudoTable {
    ...
    fetchItem(id) {
        ...
        if (this.index.has(id)) {
            ...
            const item = this.refTable.index.get(id);
            this.index.set(id, item);
            const copy = cloneDeep_1.default(item);
            return copy;
        }
...
exports.PseudoTable = PseudoTable;
class Transaction {
