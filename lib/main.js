...
class Database {
    constructor(filename, directory, snapshotInterval) {
        ...
        this.index = new Map();
        this.snapshotInterval = ms_1.default(snapshotInterval);
    ...
    save(forceSnapshot) {
        ...
        if (fs_1.existsSync(this.old)) {
            const oldStats = fs_1.statSync(this.old);
            const modified = oldStats.mtime;
            const current = new Date();
            if ((current.valueOf() - modified.valueOf() >= this.snapshotInterval
                && current.valueOf() - this.lastSnapshotTimestamp >= this.snapshotInterval) || forceSnapshot === true) {
                const snapshotFilename = ''.concat(this.directory, '/', this.filename, '_', moment_1.default(current).format('DDMMMY_hh_mm_ss_A_x'), '.rrdb.old');
                fs_1.renameSync(this.old, snapshotFilename);
